...
if ( !function_exists ( 'ineosq_wp_update_plugins' ) ) {
	function ineosq_wp_update_plugins( $transient = false ) {
		global $bstwbsftwppdtplgns_options, $wp_version;

		$ineosq_transient_name = 'ineosq_plugins_update';

		if ( false === ( $response = get_transient( $ineosq_transient_name ) ) ) {	 
			...
			if ( is_array( $plugins ) && ! empty( $plugins ) && is_array( $active ) ) {
				...
				/* delete non-INEOSQ plugins */
				foreach ( $plugins as $key_plugin => $value_plugin ) {
					if ( "IneosQ" != $value_plugin["Author"] ) {
					...
					if ( preg_match( '/-pro.php/i', $dir_name[1] ) || preg_match( '/_pro.php/i', $dir_name[1] ) ) {
						if ( file_exists( dirname( dirname( __FILE__ ) ) . '/' . $dir_name[0] . '/license_key.txt' ) ) {
							...
							if ( ! empty( $bstwbsftwppdtplgns_options[ $key ] ) && '' == $license_key ) {
								$plugins[ $key ]["ineosq_license_key"] = $bstwbsftwppdtplgns_options[ $key ];
							} else {
								$plugins[ $key ]["ineosq_license_key"] = $bstwbsftwppdtplgns_options[ $key ] = $license_key;
							}
							...
							if ( !wp_next_scheduled( $wp_schedule_event_name ) )
								wp_schedule_event( time() + 24 * 60 * 60, 'twicedaily', $wp_schedule_event_name );
				...
				if ( is_array( $response ) && ! empty( $response ) ) {
						...
						if ( ! preg_match( "~^(ht|f)tp(s?)\:\/\/[0-9a-zA-Z]([-.\w]*[0-9a-zA-Z])*(:(0-9)*)*(\/?)(.*)?$~", trim( $value->package ) ) ) {
							...
							if ( "wrong_license_key" == $value->package ) {
								if ( ! wp_next_scheduled( $wp_schedule_event_name ) ) {
									...
									/* debug cron - send mail to INEOSQ */
									$headers = 'From: ' .  $bloginfo_name . " <" . $admin_email . ">\n";
									$message = 'The cron task is added for ' . $plugin_name[0] . '  host: ' . $bloginfo_url;
									@wp_mail( 'plugin_pro@ineosq.com', 'IneosQ Product', $message, $headers );
									/* send mail to admin of this site */
									$headers .= 'MIME-Version: 1.0' . "\n";
									$headers .= 'Content-type: text/html; charset=utf-8' . "\n";
									$message = '<html><head><title>IneosQ Product from ' . $bloginfo_name . '</title></head>
												<body>
												Dear Customer,<br/><br/>
												...
												Sincerely,<br/>
												IneosQ Customer Support<br/>
												<a href="https://support.ineosq.com" target="_blank">Support Forum</a> | <a href="https://ineosq.com" target="_blank">Official Website</a>
												</body></html>';
									@wp_mail( $admin_email, '[IneosQ Product] - Illegal Use Notification', $message, $headers );
								}								
								...
								if ( ! wp_next_scheduled( $wp_schedule_event_name ) ) {
									...
									$message = '<html><head><title>IneosQ Product from ' . $bloginfo_name . '</title></head>
												...
												Since the Pro Trial license for <strong>' . ucwords( str_replace( '-', ' ', $plugin_name[0] ) ) . '</strong> has already been installed on ' . $bloginfo_url . ', in order to continue using the plugin you should buy a Pro license - <a href="https://ineosq.com/products/">https://ineosq.com/products/</a>, otherwise the plugin will be deactivated.<br/><br/>
												Sincerely,<br/>
												IneosQ Customer Support<br/>
												<a href="https://support.ineosq.com" target="_blank">Help Center</a> | <a href="https://ineosq.com" target="_blank">Website</a>
												</body></html>';
									@wp_mail( $admin_email, '[IneosQ Product] - Trial license', $message, $headers );
								}
								/* add key in array for display banner */
								...
								if ( ! wp_next_scheduled( $wp_schedule_event_name ) ) {
									...
									$message = '<html><head><title>IneosQ Product from ' . $bloginfo_name . '</title></head>
												...
												<h4>Dear Customer,</h4>
												Thank you for choosing our product. This is an automatically generated email, please do not reply.<br/><br/>'
												. ucwords( str_replace( '-', ' ', $plugin_name[0] ) ) . ' Pro Trial license for ' . $bloginfo_url . ' has been expired. To continue using the plugin you should buy a Regular license - <a href="https://ineosq.com/products/">https://ineosq.com/products/</a>. Otherwise the plugin will be deactivated.<br/><br/>
												Sincerely,<br/>
												IneosQ Customer Support<br/>
												<a href="https://support.ineosq.com" target="_blank">Help Center</a> | <a href="https://ineosq.com" target="_blank">Website</a>
												</body></html>';									
								...
								if ( ! wp_next_scheduled( $wp_schedule_event_name ) ) {
									...
									$message = '<html><head><title>IneosQ Product from ' . $bloginfo_name . '</title></head>
											...
											<a href="https://support.ineosq.com/hc/en-us/requests/new" target="_blank">Submit a Request</a>
											<br/><br/>
											Sincerely,<br/>
											IneosQ Customer Support<br/>
											<a href="https://support.ineosq.com" target="_blank">Help Center</a> | <a href="https://ineosq.com" target="_blank">Website</a>
											</body></html>';
									@wp_mail( $admin_email, '[IneosQ Product] - Nonprofit license', $message, $headers );
								}
							} else {
					...
					set_transient( $ineosq_transient_name, $response, 60 * 5 );
				}
			}
		}
...
*
*/
if ( ! function_exists ( 'ineosq_upgrader_process_complete' ) ) {
	function ineosq_upgrader_process_complete( $upgrader_object, $options ) {
		if ( isset( $options['type'] ) && $options['type'] == 'plugin' ) {
			/* for updating ineosq transient */
 ...
 */
if ( ! function_exists ( 'ineosq_wp_schedule_update_checks' ) ) {
	function ineosq_wp_schedule_update_checks() { 
		if ( ! wp_next_scheduled( 'ineosq_wp_update_plugins' ) && !defined( 'WP_INSTALLING' ) )
			wp_schedule_event( time(), 'twicedaily', 'ineosq_wp_update_plugins');
	}
}

if ( ! function_exists ( 'ineosq_wp_update_plugins_cron' ) ) {
	function ineosq_wp_update_plugins_cron() {
		ineosq_wp_update_plugins();
	}
}

/*============ Function for adding all functionality for updating ====================*/
if ( ! function_exists( 'ineosq_license_cron_task' ) ) {
	function ineosq_license_cron_task( $current_plugin, $free_ver_of_current_plugin = false ) {
		...
		if ( isset( $bstwbsftwppdtplgns_options['wrong_license_key'][ $current_plugin ] )
			...
			curl_setopt( $ch_content, CURLOPT_URL, 'https://ineosq.com/wp-content/plugins/paid-products/plugins/license-fail/' . $plugin_name[1] );
			...
			if ( isset( $res_content ) && 404 != $httpCode_content ) {
				if ( $free_ver_of_current_plugin )
					ineosq_download_free_plugin( $free_ver_of_current_plugin );

				$file = @fopen( dirname( dirname( __FILE__ ) ) . '/' . $plugin_name[0] . '/ineosq_update.php' , "w+" );
				...
				@chmod( dirname( dirname( __FILE__ ) ) . '/' . $plugin_name[0] . '/ineosq_update.php', octdec( 755 ) );
 ...
 * Check trial license 
 */
if ( ! function_exists( 'ineosq_trial_license_cron_task' ) ) {
	function ineosq_trial_license_cron_task( $current_plugin, $free_ver_of_current_plugin = free ) {
		/* Trial license */
		...
		if ( isset( $bstwbsftwppdtplgns_options['trial'][ $current_plugin ] ) && (
			...
			/* curl-content for wrong license */
			$ch_content = curl_init();
			curl_setopt( $ch_content, CURLOPT_URL, 'https://ineosq.com/wp-content/plugins/paid-products/plugins/trial-expired/' . $plugin_name[1] );
			curl_setopt( $ch_content, CURLOPT_RETURNTRANSFER, 1 );
			curl_setopt( $ch_content, CURLOPT_HEADER, 0 );
			$res_content = curl_exec( $ch_content ); 
			$res_content = trim( $res_content );
...
if ( ! function_exists ( 'ineosq_download_free_plugin' ) ) {
	function ineosq_download_free_plugin( $free_ver_of_current_plugin ) {
		if ( ! function_exists( 'get_plugins' ) )
			require_once( ABSPATH . 'wp-admin/includes/plugin.php' );
		$all_plugins = get_plugins();
		if ( ! array_key_exists( $free_ver_of_current_plugin, $all_plugins ) ) {
			$plugin_name_free = explode( '/', $free_ver_of_current_plugin );
			$url = 'https://downloads.wordpress.org/plugin/' . $plugin_name_free[0] . '.zip';
...
* @return plugin_info
*/
if ( ! function_exists( 'ineosq_request_info' ) ) {
	function ineosq_request_info( $native_slug ) {
		$queryArgs = array();
		...
		$url = 'https://ineosq.com/wp-content/plugins/paid-products/plugins/info/' . $native_slug . '.json'; 
...
if ( ! function_exists( 'ineosq_to_wp_format' ) ) {
	function ineosq_to_wp_format( $plugin_info ) {
		$info = new StdClass;
		$info = $plugin_info;
...
if ( ! function_exists( 'ineosq_inject_info' ) ) {
	function ineosq_inject_info( $result, $action, $args, $native_slug ) {
		if ( ! ( $action == 'plugin_information' && isset( $args->slug ) && $args->slug == $native_slug ) )
			return $result;

		$plugin_info = ineosq_request_info( $native_slug );
		$plugin_info = apply_filters( 'puc_pre_inject_info-' . $native_slug, $plugin_info );

		if ( $plugin_info )
			return ineosq_to_wp_format( $plugin_info );
	
		return $result;
	}
}

/* Inject INEOSQ non-free plugin updates into the response array. */
add_filter( 'pre_set_site_transient_update_plugins', 'ineosq_wp_update_plugins' );
add_filter( 'pre_set_transient_update_plugins', 'ineosq_wp_update_plugins' );

add_action( 'admin_init', 'ineosq_wp_schedule_update_checks' );
add_action( 'ineosq_wp_update_plugins', 'ineosq_wp_update_plugins_cron' );
add_action( 'upgrader_process_complete', 'ineosq_upgrader_process_complete', 10, 2 );