...
if ( ! class_exists( 'Cptch_Package_Loader' ) ) {
	class Cptch_Package_Loader {
		...
		private $error,			/* string, message about the errors, which have occurred during the action handling */
			...
			$upload_dir,		/* string, absolute path to the 'ineosq_captcha_images' folder in the 'uploads' folder */
		...
		function __construct() {
			...
			$this->upload_dir = $upload_dir['basedir'] . '/ineosq_captcha_images';
			if( ! file_exists( $this->upload_dir ) ) {
				if ( is_writable( $upload_dir['basedir'] ) )
					mkdir( $this->upload_dir );
				else
					$this->error = __( 'Can not load images in to the "uploads" folder. Please, check your permissions.', 'ineosq-captcha-pro' );
			}
		...
		function display() {
			...
			<div class="upload-plugin cptch_install_package_wrap">
				<form id="cptch_install_package_form" class="ineosq_form wp-upload-form" method="post" enctype="multipart/form-data" action="admin.php?page=captcha-packages.php&amp;cptch_action=new">
					<fieldset>
						<p>
							<label>
						...
						<p><?php _e( 'If the package already exists', 'ineosq-captcha-pro' ); ?></p>
						<p>
							...
							<label for="cptch_skip_existed">
								...
								<?php _e( 'Skip it', 'ineosq-captcha-pro' ); ?>
							</label><br/>
							<label for="cptch_update_existed">
								...
								<?php _e( 'Update the existed one', 'ineosq-captcha-pro' ); ?>
							</label><br/>
							<label for="cptch_save_as_new_existed">
								...
								<?php _e( 'Save it as new', 'ineosq-captcha-pro' ); ?>
							</label>
						</p>
						<p>
							<label>
								<input class="button-primary cptch_install_disabled" name="cptch_install_package_submit" type="submit" value="<?php _e( 'Install Now', 'ineosq-captcha-pro' ); ?>" />
							</label>
							<a class="cptch_add_ons_link" href="https://ineosq.com/products/wordpress/plugins/captcha/#addons" target="_blank"><?php _e( 'Browse packages', 'ineosq-captcha-pro' ); ?></a>
						</p>
		...
		private function upload_archive() {
			...
			$error_part		= __( "Error during package uploading", 'ineosq-captcha-pro' ) . ':&nbsp;';
			...
			if ( ! is_uploaded_file( $file['tmp_name'] ) )
				$this->error = $error_part . __( "check your archive", 'ineosq-captcha-pro' ) . '.';
			elseif ( ! $is_zip )
				$this->error = $error_part . __( "file format should be ZIP-archive", 'ineosq-captcha-pro' ) . '.';
			elseif ( $file['size'] > $max_size )
				$this->error = $error_part . __( "file size should not exceed", 'ineosq-captcha-pro' ) . $this->get_human_readeble_file_size( $max_size ) . '.';
			...
			if( ! move_uploaded_file( $file['tmp_name'], $copied ) ) {
				$this->error = $error_part . __( "it is impossible to upload the archive to the server", 'ineosq-captcha-pro' ) . '.';
				return false;
			}
			...
			if ( $unzip_result ) {
				...
				$this->error = is_wp_error( $unzip_result ) ? $unzip_result->get_error_message() : $error_part .  __( "archive can not be unzipped", 'ineosq-captcha-pro' ) . '.';
			}
			return true;
		}
		...
		private function get_human_readeble_file_size( $bytes ) {
			if ( 104857 <= $bytes ) /* if file size more then 100KB */
				return round( $bytes / 1048576, 2 ) . "&nbsp;" . __( 'MB', 'ineosq-captcha-pro' );
			if ( 1024 <= $bytes && 104857 >= $bytes ) /* if file size more then 1KB but less then 100KB */
				return round( $bytes / 1024, 2 ) . "&nbsp;" . __( 'KB', 'ineosq-captcha-pro' );
			/* if file size under 1KB */
			return $bytes . "&nbsp;" . __( 'Bytes', 'ineosq-captcha-pro' );
		}
		...
		function save_packages( $pakages_dir = '', $remove_package = true, $existed_action = '' ) {
			...
			if ( $remove_package ) {
				...
				if ( ! $this->error ) {
					$packages_message	= sprintf( _n( 'One package has been updated or added to the database.', '%s packages have been updated or added to the database.', $this->result[0], 'ineosq-captcha-pro' ), $this->result[0] );
					$images_message		= sprintf( _n( 'One image has been updated or added to the database.', '%s images have been updated or added to the database.', $this->result[1], 'ineosq-captcha-pro' ), $this->result[1] );
		...
		private function parse_folders() {
			...
			if ( 2 >= count( $files ) ) {
				$this->error = __( 'Archive is empty', 'ineosq-captcha-pro' );
				return false;
			}
		...
		function check_settings( $settings, $package_name = '', $package_id = '', $serialize = true ) {
			...
			if ( ! empty( $wrong ) ) {
				$message =
					sprintf(
						__( 'Some settings of the package %s were set incorrectly. They have been skipped.', 'ineosq-captcha-pro' ),
						"<a href=\"admin.php?page=captcha-packages.php&cptch_action=edit&id={$package_id}\">\"{$package_name}\"</a>"
					) . '<br />' .
					__( 'Wrong data', 'ineosq-captcha-pro' ) . ':&nbsp;' . implode( ',&nbsp;', $wrong );
