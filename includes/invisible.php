	...
	class Cptch_Invisible {
		...
		public function __construct() {
			...
			try {
				...
				if ( ! $this->use_openssl && ! function_exists( 'mcrypt_encrypt' ) )
					throw new Exception( __( "Can't handle encrypting/decrypting data.", 'ineosq-captcha-pro' ), 'cant_crypt' );

			} catch( Exception $e ) {
				$this->add_error( 'cptch_init_error', __( 'ERROR', 'ineosq-captcha-pro' ) . ': ' . $e->getMessage() );
			}
		...
		public function get_content() {
			...
			try {
				...
				return "<input type=\"hidden\" name=\"cptch_code\" value=\"{$code}\"><input type=\"hidden\" name=\"cptch_key\" value=\"{$key}\">" . sprintf( __( "Protected by %s", 'ineosq-captcha-pro' ), 'IneosQ Captcha' );

			} catch( Exception $e ) {
				$this->add_error( $e->getCode(), __( 'ERROR', 'ineosq-captcha-pro' ) . ': ' . $e->getMessage() );
				return false;
			}
		}
		...
		public function check() {
			try {
				...
				if ( empty( $code ) || empty( $key ) )
					throw new Exception( __( 'Empty captcha.', 'ineosq-captcha-pro' ) );
				...
				if ( absint( $answer['time'] ) + $this->data['time_limit'] < $this->data['current_time'] )
					throw new Exception( __( "Captcha time limit exceeded.", 'ineosq-captcha-pro' ) );

				if ( 0 !== strcasecmp( $answer['url'], $this->data['url'] ) ||
					0 !== strcasecmp( $answer['secret'], $this->data['secret_key'] )
				)
					throw new Exception( __( "Wrong captcha.", 'ineosq-captcha-pro' ) );

			} catch( Exception $e ) {
				$this->add_error( 'cptch_check_errors', __( 'ERROR', 'ineosq-captcha-pro' ) . ': ' . $e->getMessage() );
			}
		}
