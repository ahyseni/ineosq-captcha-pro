...
class App extends Component {
		...
		if ( this.state.offsetX > 0 ) {
			...
			if ( this.state.offsetX >= this.maxSlidedWidth ) {
		        ...
		        .then( response => {
					...
					document.querySelector('[name="cptch_result"]').value = this.state.cptchResult;
    ...
    render() {
	   	...
	   	const colorSlide = this.state.isTouchEndSpan && ! this.state.isLoader ? wpSlideCaptcha.color_end_slide : wpSlideCaptcha.color_start_slide;
		const colorContainerSlide = wpSlideCaptcha.color_container_slide;
		const colorTextSlide = wpSlideCaptcha.color_text_slide;
		const fontSizeText = wpSlideCaptcha.font_size_text_slide;
		...
		let sliderStyle;
		if ( document.dir === "rtl" ) {
			sliderStyle = { right: this.state.offsetX + 'px', background: colorSlide };
		} else {
			sliderStyle = { left: this.state.offsetX + 'px', background: colorSlide };
		}

		let containerStyle = { background: colorContainerSlide };
		let textStyle = { color: colorTextSlide, fontSize: fontSizeText + 'px' };
	    ...
	    return (
			<div id="cptch_slide_container"
			    ...
			    <p id="slide-title" style={textStyle}>{textSlide}</p>
